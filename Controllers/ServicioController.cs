...
[Route("api/[controller]")]
    ...
    class ServicioController : ControllerBase
     {
  ...
  [HttpGet]
        ...
        {
            var listServicios = await _context.Servicios2.ToListAsync();
        ...
        public async Task<IActionResult> Update([FromBody] Servicios servicios2)
        {
            ...
            var entity = await _context.Servicios2.FindAsync(servicios2.Id);
            if (entity == null)
            {
        ...
        {
            var producto = await _context.Servicios2.FindAsync(id);
            if (producto == null)
            {
                return NotFound();
            }
            _context.Servicios2.Remove(producto);
            await _context.SaveChangesAsync();
            return Ok();
        }
